...
from core.permissions import IsDeputyLeaderOfCommunity, IsInActiveCommunity
...
class NotExistingQRCodeSerializer(QRCodeSerializerTemplate):
    ...
    def validate(self, data, get_errors=False):
        ...
        if not IsDeputyLeaderOfCommunity().has_object_permission(self.context['request'], None, data['event']):
            add_error_message(errors, message='QR codes must only be created by a deputy leader of the community.')

        if not IsInActiveCommunity().has_object_permission(self.context['request'], None, data['event']):
            add_error_message(
                errors, key='event', message='QR codes are not able to be created in non-active events.'
            )
...
class NotExistingJoinKeySerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not IsInActiveCommunity().has_object_permission(self.context['request'], None, data['event']):
            add_error_message(
                errors, key='event', message='Join keys are not able to be created in non-active events.'
            )
...
class NotExistingGeneratedDocxSerializer(GeneratedDocxSerializerTemplate):
    ...
    def validate(self, data, get_errors=False):
        ...
        if not IsInActiveCommunity().has_object_permission(self.context['request'], None, data['club']):
            add_error_message(
                errors, key='club',
                message='Club approval and renewal documents are not able to be created in non-active clubs.'
            )
